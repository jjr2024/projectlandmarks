...
│   ├── auth/                  Sign in/up (with ← Home link; initial mode from `?mode=signup` query param — marketing CTAs link there), callback, confirm (+confirm/verify, token_hash flow), forgot-password, reset-password
│   └── api/
...
├── 025 add_onboarding_completed: add onboarding_completed boolean (NOT NULL DEFAULT false) to profiles, backfill all existing users to true. Drives server-side onboarding routing (Architecture § "Onboarding routing"). Replaced the old client-side account-age + contact-count heuristic.
scripts/
...
**Onboarding routing (`profiles.onboarding_completed`):** Whether a user is sent to `/onboarding` vs the app is decided by a **single durable boolean**, `profiles.onboarding_completed` (migration 025; default `false`, `NOT NULL`). Enforcement is **server-side** in `(app)/layout.tsx` — after the consent gate, `if (!profile.onboarding_completed) redirect("/onboarding")` — so it covers every route under `(app)` (dashboard, contacts, settings) with no client flash. `(onboarding)/layout.tsx` does the inverse: completed users hitting `/onboarding` are redirected to `/dashboard`. The flag is set to `true` exactly once, in `handleSaveAndFinish` (`onboarding/page.tsx`), **after** the first contact + events inserts succeed (inside the `try`, so a failure surfaces via `friendlyError()` instead of leaving the user un-onboarded). Consequences: (1) a user **cannot reach the app without adding a first contact** — there is no skip path, by product decision; (2) once completed, the flag never flips back, so **deleting your only contact does NOT re-trigger onboarding** — you land on the empty-state dashboard. New signups get `false` via the column default (`handle_new_user()` doesn't set the column); all pre-migration users were backfilled to `true`. **This replaced an unreliable client-side heuristic** (account created in last hour + zero contacts, checked only in `dashboard/page.tsx`) which stranded users who returned after the 1-hour window — do not reintroduce time/contact-count inference.

**Onboarding flow:** 4-step flow (Welcome → Contact+Events → Gift prefs → Done). Always starts at step 1 — **URL-based step initialization (`?step=`) was intentionally removed** because it allowed users to jump to step 3/4 with blank form state, causing garbage contact inserts or RLS errors. Do not re-add it; the flow is short enough that starting from step 1 is fine. Page shows a loading spinner until profile fetch resolves (prevents "Welcome, !" flash). `savedRef` prevents double-submission if the user navigates Back from step 4. The step-4 "Go to dashboard" / "Add more contacts" buttons use a **full navigation (`window.location.href`), not `router.push`** — a soft nav can replay the stale RSC payload from `(app)/layout.tsx` captured before `onboarding_completed` flipped, bouncing the user back into onboarding. `force-dynamic` on the layouts only defeats the server Data Cache, not the client Router Cache; same workaround as the consent page. Do not revert to `router.push` here. Error messages use `friendlyError()`. Collects full contact fields including notes. Events include "Other options" expandable (year_started, one_time, event_year) — collapsed by default to keep the happy path clean. If `one_time` is checked, `event_year` becomes required (red border + inline error if empty, blocks save). Checking `one_time` auto-populates `event_year` with the current year and keeps the collapsible expanded. Unchecking clears `event_year`. The contact detail event modal (`contacts/[id]`) has the same validation. Icons use inline SVGs (no emojis). "Skip gifts" is a toggle button, not a checkbox.
...
**Auth:** Supabase Auth with PKCE flow (`@supabase/ssr` default). Email verification required — no emails sent to unverified addresses (GDPR/CAN-SPAM: emails contain affiliate links). Password changes use `reauthenticate()` (nonce-based, no duplicate session). Post-signup success screen and in-app `EmailVerificationBanner` both offer resend-verification with 60-second cooldown (persisted to `sessionStorage`). **Email confirmation uses the `token_hash` flow, NOT the PKCE `code` flow.** The Confirm-signup email links to `/auth/confirm?token_hash=…&type=signup&next=/dashboard` (Supabase template builds the URL from `{{ .SiteURL }}` + `{{ .TokenHash }}`). `/auth/confirm` (`auth/confirm/page.tsx`) is an interstitial page that renders a "Confirm my email" button; clicking it POSTs to `/auth/confirm/verify` (`auth/confirm/verify/route.ts`), which calls `supabase.auth.verifyOtp({ type, token_hash })` server-side and sets the session on a **303** redirect to the validated `next`. Because `verifyOtp` needs no `code_verifier`, the link verifies AND signs the user in on **any device** — fixing the cross-device dead-end the PKCE `?code=` flow had. The interstitial (no verify-on-GET) stops email scanners/prefetchers from consuming the single-use token before the human clicks. `resend()` is now safe for verification emails — it mints a fresh `token_hash` link that works cross-device (no PKCE pair to regenerate). Shared open-redirect guard `validateNext()` and the OTP-type allowlist `isAllowedOtpType()` live in `lib/auth-callback.ts`. `/auth/callback` (PKCE `exchangeCodeForSession`) is **retained only** for OAuth (when re-enabled) and any in-flight old-format links; its Option B "verified vs expired" relabeling (`resolveCallbackRedirect`) still applies there. **Password reset still uses PKCE** (`exchangeCodeForSession` in `reset-password/page.tsx`) — unifying it onto `token_hash` is the deferred Phase 2. See `docs/auth-callback-option-a-plan.md` (Option A) and `docs/auth-callback-option-b-plan.md` (Option B).
...
| Table | Key columns |
|---|---|
| `profiles` | display_name, timezone, preferred_send_hour, reminder_days_before, drips_sent, consent_terms, consent_emails, email_reminders_enabled, monthly_digest_enabled, last_digest_sent, onboarding_completed, signup_source, **updated_at, last_changed_fields** |
...
| `/auth/callback` | GET | — | OAuth + in-flight old-format links → session (PKCE; Option B fallback: redirects to app if exchange fails but session exists) |
| `/auth/confirm` | GET (page) | token_hash | Interstitial confirm page — renders "Confirm my email" button (no verify-on-GET, scanner-safe) |
| `/auth/confirm/verify` | POST | token_hash | `verifyOtp({type,token_hash})` → session on 303 redirect to `next`; device-independent email confirmation |
...
- Auth page initial mode: `auth/page.tsx` reads `?mode=` from `searchParams` to set the initial `mode` state (`signup` → sign-up form, anything else/missing → sign-in). All marketing "Get started" / "Create account" CTAs link to `/auth?mode=signup`. Unlike onboarding (where `?step=` was intentionally removed), reading `?mode=` here is safe — it only selects which form renders, not partial form state. Don't hardcode the initial mode back to `"signin"`.
- Domain: `daysight.xyz`
...
- **No instant/same-day delivery claims in emails.** Daysight doesn't control fulfillment (affiliate links), so reminder copy must not promise instant or same-day delivery. The last-minute banner (`reminder.tsx`, `isLastMinute = daysBefore <= 2`, fires on the 0/1/2-day reminders) and the 1-day intro use the softer "tend to ship quickly, so there's still time" phrasing. Don't reintroduce "delivers instantly" / "ships today" / "same-day options."
...
- **Verification emails use the `token_hash` flow — `resend()` is safe.** Email confirmation links to `/auth/confirm` and is verified via `verifyOtp({ type, token_hash })` server-side, which needs no PKCE `code_verifier`, so `resend()` produces a link that works cross-device. (This replaced the old PKCE caveat: `resend()` previously couldn't regenerate the PKCE pair, so re-sent `?code=` links failed at `exchangeCodeForSession()`.) The PKCE caveat **still applies to password reset** (`reset-password/page.tsx`) until Phase 2. See `handleResendVerification` in `auth/page.tsx` and `auth/confirm/verify/route.ts`.
